...
class RealConv(nn.Module):
    def __init__(self, num_out_chnnl):
        super(RealConv, self).__init__()
        self.conv = nn.Conv2d(1, num_out_chnnl, (3, 2), padding='same', bias=False)
...
if __name__ == "__main__":
    # 数据预处理
    file_addr = '../Data/'
    file_name = 'Simulation_MPDPD_Data_00'
    ...
    dict = {}
    for num_neuron in range(1, 10):
        model = MyModel(num_neuron).to(device)
        param_num = sum([param.nelement() for param in model.parameters()]) * 2
        if num_neuron > 1:
            model_tmp = torch.load('../Models/' + file_name + '_GNAS-CVCNN_' + str(num_neuron - 1) + '.pth')
        ...
        scio.savemat('../DataSave/' + file_name + '_GNAS-CVCNN_' + str(num_neuron) + '.mat', dict)
        torch.save(model, '../Models/' + file_name + '_GNAS-CVCNN_' + str(num_neuron) + '.pth')
